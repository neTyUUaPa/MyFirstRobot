...
float integralEnc = 0, previousErrorEnc = 0;
float lastTimeEnc = 0;
int moveForwardPid(){
  static float kp = 13.0, ki = 1.2, kd = 5.0;  // Настроенные параметры PID
  unsigned long currentTime = millis();
  float dt = (currentTime - lastTimeEnc) / 1000.0;  // Время в секундах
  long* encodersData = getEncodersData();
  float error = abs(encodersData[0]) - abs(encodersData[1]);
  //Serial.println(encodersData[0]);
  //Serial.println(encodersData[1]);
  lastTimeEnc = currentTime;
  //float error = GetEncError();

  if (dt <= 0) dt = 0.001;  // Минимальный dt для избежания деления на ноль
  float derivative = (error - previousErrorEnc) / dt;
  integralEnc += error * dt;  // Учитываем dt для корректного накопления
  float output = kp * error + ki * integralEnc + kd * derivative;
  previousErrorEnc = error;

  //Serial.print("PID - P: ");
  //Serial.print(kp * error, 2);
  //Serial.print(", I: ");
  //Serial.print(ki * integralEnc, 2);
  //Serial.print(", D: ");
  //Serial.print(kd * derivative, 2);
  //Serial.print(", Output: ");
  //Serial.println(output, 2);
  return constrain(output, -150, 150);  // Увеличили диапазон для большей коррекции
}
...
void initializeGraph() {
    ...
    // C1 <-> C2
    graph[0][1][0] = 148.5;  // Расстояние
    graph[0][1][1] = 90.0;  // Угол
    graph[1][0][0] = 148.5;  // Симметричная связь
...
float TurnDistance(int alpha){
  float k = 0.98;
  int radius = 11;
  float length = ((2 * radius * pi) / 360) * alpha * k;
  return length;
}
...
void TurnAround(int tarAngle){
  ...
  if (CheckSign(ang) == -1){ //налево
    while ((getEncodersData()[0] < lastLeftValueEncoder - (TurnDistance(ang) * oneSm)) && (getEncodersData()[1] > lastRightValueEncoder + (TurnDistance(ang) * oneSm))){
      Ride(0, 255, 1, 0);
      Serial.println("TUT");
    } 
  }
  if (CheckSign(ang) == 1){ // направо
